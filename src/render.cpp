...
static void renderPlayer() {
    ...
    if (player.is_invulnerable && ((SDL_GetTicks() - player.invuln_start_time) / 150) % 2 == 0) return;
...
// Vẽ chữ nổi điểm số (render trực tiếp, không pre-render)
...
static void renderHUD() {
    ...
    renderText(std::to_string(currentStage) + "-" + std::to_string(currentWave), {255, 230, 100, 255}, SCREEN_WIDTH - 45, 10);
    renderText("POWER LV " + std::to_string(player.weapon_level), {100, 255, 255, 255}, 10, SCREEN_HEIGHT - 35);
    renderText("LIVES: " + std::to_string(player.lives), {255, 80, 80, 255}, SCREEN_WIDTH - 95, SCREEN_HEIGHT - 35);
}
...
void render() {
    ...
    switch (currentState) {
        ...
        case GameState::GAME_WON: renderGameWon(); break;
        case GameState::PAUSED:
            renderEntities(); renderPlayer(); renderFloatingTexts();
